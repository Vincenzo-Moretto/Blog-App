...
const PostDetails = () => {
  ...
  const fetchPostComments = async () => {
    ...
    try {
      ...
      setLoader(false); // Nasconde il loader in ogni caso
    }
  };
  ...
  // Funzione per postare un commento
  const postComment = async (e) => {
    ...
    try {
      ...
      fetchPostComments(); // Ricarica i commenti senza ricaricare la pagina
    } catch (err) {
      console.log(err);
    }
  };
  ...
  const refreshComments = () => {
    fetchPostComments(); // Ricarica i commenti dal server
  };
      ...
      {loader ? (
        ...
        <div className="px-8 md:px-[200px] mt-8">
          ...
          <div className="flex flex-col mt-4">
            ...
            {comments?.map((c) => (
              <Comment key={c._id} c={c} refreshComments={refreshComments} />
            ))}
          </div>
          {/* Area per scrivere un commento */}
          <div className="w-full flex flex-col mt-4 md:flex-row">
            <input
              onChange={(e) => setComment(e.target.value)}
              type="text"
              value={comment} // Imposta il valore dell'input
