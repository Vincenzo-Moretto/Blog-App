...
const Register = () => {
  ...
  const handleRegister = async () => {
    try {
      const url = `${process.env.REACT_APP_API_BASE_URL}/api/auth/register`; // Usa la variabile d'ambiente corretta

      const res = await axios.post(url, { username, email, password }); // Usa l'URL dinamico
      console.log(res); // Logga la risposta per capire cosa succede
      ...
      if (err.response) {
        ...
        console.log("Errore nel backend:", err.response.data);
      } else if (err.request) {
